...
func Execute() error {
	...
	rootCmd := &cobra.Command{
		...
		Short:        "Quotes, charts, fundamentals, options, and portfolio tracking against Yahoo Finance — no API key, works offline once synced",
		SilenceUsage: true,
		Version:      version,
	}
	...
	rootCmd.AddCommand(newQuotePromotedCmd(&flags))
	...
	// Transcendence commands — unique to yahoo-finance-pp-cli
	...
	// Add login-chrome subcommand to the generated auth command, if present
	for _, sub := range rootCmd.Commands() {
		if sub.Use == "auth" {
			sub.AddCommand(newAuthChromeCmd(&flags))
			break
		}
	}
