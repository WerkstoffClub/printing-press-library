...
type Config struct {
	...
	Path           string            `toml:"-"`
	SongkickApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SONGKICK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "songkick-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SONGKICK_API_KEY"); v != "" {
		cfg.SongkickApiKey = v
		cfg.AuthSource = "env:SONGKICK_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SongkickApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SONGKICK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":            token,
		"api_key":          c.SongkickApiKey,
		"SONGKICK_API_KEY": c.SongkickApiKey,
	}
