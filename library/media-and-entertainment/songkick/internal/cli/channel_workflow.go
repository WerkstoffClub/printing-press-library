...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  songkick-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  songkick-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"find", "find-locations-json"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  songkick-pp-cli workflow status

  # Show status as JSON
  songkick-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("songkick-pp-cli")
			}
