...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".songkick-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SONGKICK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SONGKICK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "songkick-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.songkick-pp-cli/feedback.jsonl.
When ` + "`SONGKICK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SONGKICK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  songkick-pp-cli feedback list
  songkick-pp-cli feedback list --limit 5
  songkick-pp-cli feedback list --json`,
