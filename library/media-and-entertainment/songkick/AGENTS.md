# Songkick Printed CLI Agent Guide

This directory is a generated `songkick-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
songkick-pp-cli doctor --json
songkick-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
songkick-pp-cli which "<capability>" --json
songkick-pp-cli <command> --help
```
...
```bash
songkick-pp-cli <command> --help
songkick-pp-cli <command> --dry-run --agent
```
