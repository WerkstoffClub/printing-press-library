...
// typed v2 table. Regression for issue #268: before the fix, paginated
...
func TestUpsertBatch_PopulatesV2Table(t *testing.T) {
	...
	if err := s.UpsertBatch("v2", items); err != nil {
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "v2").Scan(&generic); err != nil {
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "v2")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count v2: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("v2 count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
