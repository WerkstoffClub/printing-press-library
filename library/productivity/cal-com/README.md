...
- **Named profiles** — save a set of flags under a name and reuse them: `cal-com-pp-cli profile save <name> --<flag> <value>`, then `cal-com-pp-cli --profile <name> <command>`. Flag precedence: explicit flag > env var > profile > default.
...
- **`feedback`** — record in-band feedback about the CLI. Entries append as JSON lines to `~/.cal-com-pp-cli/feedback.jsonl`. When `CAL_COM_FEEDBACK_ENDPOINT` is set and either `--send` is passed or `CAL_COM_FEEDBACK_AUTO_SEND=true`, the entry is also POSTed upstream.
